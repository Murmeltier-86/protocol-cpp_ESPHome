...
class Program {
    ...
    static void Main(string[] args) {
        ...
        var commands = xdoc.Root.Elements("command").Select(e => new {
            Name = (string)e.Attribute("name") ?? "(unnamed)",
            Code = (string)e.Attribute("code") ?? "",
            Mode = (string)e.Attribute("mode") ?? "db"
        }).Where(c => !string.IsNullOrWhiteSpace(c.Code)).ToList();
        ...
        var cmd = commands[idx]; bool plain = (cmd.Mode.ToLowerInvariant())=="plain";
