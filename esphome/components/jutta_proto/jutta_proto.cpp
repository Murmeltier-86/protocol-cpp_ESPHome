...
constexpr uint32_t kTvPModeTtlMs = 10000;
// XML ALERT bits are resolved by name when the machine XML is loaded. These
// are only fallbacks for the embedded EF532M XML schema.
...
void JuraComponent::update_machine_status_from_state_(const char *source) {
  ...
  const uint32_t now = esphome::millis();
  if (this->current_tv_pmode_active_ && this->last_tv_progress_ms_ != 0 &&
      time_reached(now, this->last_tv_progress_ms_ + kTvPModeTtlMs)) {
